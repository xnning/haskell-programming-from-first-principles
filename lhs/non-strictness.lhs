...
- Non-strictness is defined by the ability to evaluate expressions that have
  bindings which are bottom in them, as long as the bottom itself is never
  forced.

- A truly lazy language memoizes, or holds in memory, the results of all the
   functions it does evaluate, and, outside of toy programs, this tends to use
   unacceptably large amounts of memory. Implementations of Haskell, such as GHC
   Haskell, are only obligated to be **non-strict** such that they have the same
   behavior with respect to bottom; they are not required to take a particular
   approach on how the program executes or how efficiently it does so.

- What `seq` does is evaluate expressions up to _weak head normal form_. WHNF
  means it stops at the first data constructor or lambda.
...
- Case matching also chains evaluation. In haskell, case matching is strict, or
  at least the pattern matching of it is, up to WHNF. In Core, cases are always
  strict to WHNF. To see the difference, consider:
...
- A **thunk** is used to reference suspended computations that might be
    performed or computed at a later point in the program.
    + Not all values get thunked. GHC will not thunk values which are merely
    data constructors. Data constructors are known to be constant, which
    justifies the safety of the optimization.

- What promotes sharing: names.

- What prevents sharing:
    1. Inlining expressions where they get used prevents sharing because it
  creates independent thunks that will get computed separately.
    2. Being a function with explicit, named arguments also prevents sharing.
...
- Forcing evaluation: in some cases, it is cheaper to just compute something
  than to construct a thunk and then evaluate it later. This case is
