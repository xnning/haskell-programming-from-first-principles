...
- The existential variable in `SomeException` allows us to raise error values of
  two totally different types without enumerating them in a sum type.
...
- `Typeable`: exists to permit types to be known at runtime, allowing for a sort
  of dynamic typechecking.

- Typeable has one method. We don't usually call this function directly. But it
  gets called for us by the `fromException` function, and `fromException` is
  called by the `catch` function.

```haskell
cast :: (Typeable a, Typeable b) => a -> Maybe b
```

- `catch` runs only if the exception matching the type you specified gets
  thrown, and it gives you an opportunitity to recover from the error and still
